...
class MathQAExp(object):
    ...
    def process_datapoint(self, data_point, args):
        ...
        for _ in range(args.rate_of_corruption):
            ...
            temp_tgt = self.process_tgt(tgt_lst, num_in_q, args.num_incorrect_tokens)
    ...
    def create_pairs(self, data_points_lst, args):
        ...
        for item in tqdm(data_points_lst):
            new_src, new_tgt = self.process_datapoint(item, args)
            src.extend(new_src)
            tgt.extend(new_tgt)
        return src, tgt
    ...
    def main(self, args):
        ...
        src, tgt = self.create_pairs(data_points_lst, args)
        print('## Writing data ##')
        self.write_data(src, tgt, args)
        print('## Completed')
