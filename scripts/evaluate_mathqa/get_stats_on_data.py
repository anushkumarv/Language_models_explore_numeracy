...
numbers_in_wrods = {"one": 1, "two": 2, "three": 3, "four": 4, "five": 5, "six": 6, "seven": 7, "eight": 8, "nine": 9,
...
def is_number(word):
    ...
    word = re.sub(",", "", word)
    word = re.sub("\.", "", word)
    return word.isdigit() and '²' not in word and '³' not in word and '¹' not in word and ('²' not in word) and (
                '³' not in word) and ('¹' not in word) and ('₂' not in word) and ('⁶' not in word) and (
                       '₃' not in word) and '⁹' not in word and '⁵' not in word and '₁' not in word and '₄' not in word and '⁷' not in word and '⁴' not in word and '⁸' not in word and '₈' not in word
...
def read_src_tgt(test_complete_info_file_name, test_src_file_name, pred_file_name, n_best_check):
    ...
    for line in total_set_input_file:
        ...
        if len(line_parts) == 9:
            total_set_mapping[
                line_parts[-2].replace('1 / 4', '0.25').replace('1 / 2', '0.5').replace('1 / 3', '0.33')] = (
            line_parts[-1], line_parts[3], line_parts[6], line_parts[4])
        else:
            total_set_mapping[
                line_parts[0].replace('1 / 4', '0.25').replace('1 / 2', '0.5').replace('1 / 3', '0.33')] = (
            line_parts[1], line_parts[3], line_parts[4], line_parts[2])

    test_input_src_file = open(test_src_file_name, encoding="utf8")

    for line in test_input_src_file:
        test_data_src.append(
            line[:-1].lower().replace('1 / 4', '0.25').replace('1 / 2', '0.5').replace('1 / 3', '0.33'))
    ...
    for line in test_input_tgt_file:
        new_test.append(
            line[:-1].lower().replace("0_3937", "0.3937").replace('const_0_33', 'const_0.33').replace("const_0_25",
                                                                                                      'const_0.25').replace(
                "0_2778", "0.2778").replace("0_6", "0.6").replace("1_6", "1.6").replace("3_6", "3.6").replace("__",
                                                                                                              " "))
        count += 1
...
def parse_options(options):
    res_opts = []
    options = options.replace('u\'', '').replace('\"', '\'').replace('\'', '').replace(']', '').replace('[',
                                                                                                        '').replace(
        '  ', ', ').split(', ')
    for opt in options:
        if ')' not in opt:
...
def solve_procedure(test_complete_info_file_name, test_src_file_name, pred_file_name, n_best_check):
    matching_counts_with_n_candidates = dict()
    non_matching_counts_with_n_candidates = dict()
    test_data_src, test_data_tgt, test_data_info = read_src_tgt(test_complete_info_file_name, test_src_file_name,
                                                                pred_file_name, n_best_check)
    score_count = 0
    found_solution = []
    ...
    for i in range(0, len(test_data_src)):
        ...
        for ii in range(0, n_best_check):
            for ans in ans_th:
                ...
                if ans_candidates != [] and ((min_distance < best_difference and \
                                              ...
                                              (len(best_candidate_list) == 0 or len(best_candidate_list) >= len(
                                                  ans_candidates)) \
                        )):
        ...
        if best_candidate_list == []:  # if it is the last one and we have not found an answer.
            ...
            score_count += 0.2
            matching_counts_with_n_candidates[len(best_candidate_list)] = 1 + matching_counts_with_n_candidates[
                len(best_candidate_list)] if matching_counts_with_n_candidates.get(len(best_candidate_list), '') else 1
        ...
        elif int(ord(list(test_data_info[i])[1]) - ord('a')) not in best_candidate_list:
            non_matching_counts_with_n_candidates[len(best_candidate_list)] = 1 + non_matching_counts_with_n_candidates[
                len(best_candidate_list)] if non_matching_counts_with_n_candidates.get(len(best_candidate_list), '') else 1

    total_question_with_n_candidates = Counter(matching_counts_with_n_candidates) + Counter(non_matching_counts_with_n_candidates)
    print("Accuracy would be: " + str((score_count + 0.0) / len(test_data_src)))
    print("Num of data points producing correct answer with N candidate solutions : ", matching_counts_with_n_candidates)
    print("Num of data points not producing correct answer with N candidate solutions : ", non_matching_counts_with_n_candidates)
    print("Total num of data points not producing correct answer with N candidate solutions : ",total_question_with_n_candidates)
    print("len of data src: ", len(test_data_src))


def main():
    parser = argparse.ArgumentParser()

    parser.add_argument("--src_file_name", default="data_set/allcomplete_src_test.txt", type=str,
                        help="The file containing the source sentences")
    parser.add_argument("-info_file_name", default="data_set/complete_all_for_release.tsv", type=str,
                        help="The file containing the infromation of the problem.")
    parser.add_argument("--predictions", default="pred_all_separate_adam_new_adam.txt", type=str,
                        help="The path to the file containing the prediction paths")
